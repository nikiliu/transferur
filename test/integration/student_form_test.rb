...
class StudentFormTest < ActionDispatch::IntegrationTest
  def setup
    @school1 = School.create!(name: "Foo University", location: "Foo", international: false)
    @school1.courses.create!(name: "Calculus I",  course_num: "MATH 121")
    @school1.courses.create!(name: "Calculus II", course_num: "MATH 122")
    ...
    @transfer1 = TransferRequest.create!(transfer_school_id: @school1.id,
                                         transfer_course_id: @school1.courses.first.id,
                                         ur_course_id:       @school2.courses.first.id,
                                         approved:           true)

    @form_data = {
      name:            "John Doe",
      email:           "test@test.com",
      transfer_school: @school1.id,
      transfer_course: @school1.courses.first.id,
      online:          0,
      dual_enrollment: 0,
      course_number:   @school2.courses.first.id
    }
  end
